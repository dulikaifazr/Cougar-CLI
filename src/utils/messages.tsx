...
function detectAndReadImageFromPath(input: string): {
  imageBase64: string | null
  mediaType: 'image/png' | 'image/jpeg' | 'image/gif' | 'image/webp'
  cleanedInput: string
} {
  const match = input.match(/"([^"]+\.(png|jpe?g|gif|bmp|webp))"/i)
  if (!match) {
    return { imageBase64: null, mediaType: 'image/png', cleanedInput: input }
  }
  ...
  try {
    ...
    const cleanedInput = input.replace(match[0], '').trim() || '请分析这张图片'
    return { imageBase64: base64, mediaType, cleanedInput }
  } catch {
    return { imageBase64: null, mediaType: 'image/png', cleanedInput: input }
  }
}
...
export async function processUserInput(
  ...
  if (!pastedImage) {
    const detection = detectAndReadImageFromPath(input)
    if (detection.imageBase64) {
      imageToSend = detection.imageBase64
      mediaType = detection.mediaType
      finalInput = detection.cleanedInput
    }
  }
  ...
  return messages.flatMap(message => {
    ...
    return message.message.content.map(_ => {
      switch (message.type) {
        ...
        case 'user':
          // Preserve content splitting but ensure each rendered item has a unique key/uuid
          return {
            ...message,
            uuid: randomUUID(),
            message: {
