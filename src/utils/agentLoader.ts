 ...
 * prioritizing Cougar-specific overrides.
 */
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Only warn if COUGAR_DEBUG_AGENTS environment variable is set
        if (frontmatter.model && !frontmatter.model_name && !warnedAgents.has(frontmatter.name) && process.env.COUGAR_DEBUG_AGENTS) {
...
async function loadAllAgents(): Promise<{
  ...
  try {
    // Scan both .claude and .cougar directories in parallel
    // Claude Code compatibility: support both ~/.claude/agents and ~/.cougar/agents
    const userClaudeDir = join(homedir(), '.claude', 'agents')
    const userCougarDir = join(homedir(), '.cougar', 'agents')
    const projectClaudeDir = join(getCwd(), '.claude', 'agents')
    const projectCougarDir = join(getCwd(), '.cougar', 'agents')
    
    const [userClaudeAgents, userCougarAgents, projectClaudeAgents, projectCougarAgents] = await Promise.all([
      scanAgentDirectory(userClaudeDir, 'user'),
      scanAgentDirectory(userCougarDir, 'user'),
      scanAgentDirectory(projectClaudeDir, 'project'),
      scanAgentDirectory(projectCougarDir, 'project')
    ])
    
    // Built-in agents (currently just general-purpose)
    const builtinAgents = [BUILTIN_GENERAL_PURPOSE]
    
    // Apply priority override: built-in < .claude (user) < .cougar (user) < .claude (project) < .cougar (project)
    ...
    for (const agent of userCougarAgents) {
      agentMap.set(agent.agentType, agent)
    }
    for (const agent of projectClaudeAgents) {
      agentMap.set(agent.agentType, agent)
    }
    for (const agent of projectCougarAgents) {
      agentMap.set(agent.agentType, agent)
    }
    
    const activeAgents = Array.from(agentMap.values())
    const allAgents = [...builtinAgents, ...userClaudeAgents, ...userCougarAgents, ...projectClaudeAgents, ...projectCougarAgents]
...
export async function startAgentWatcher(onChange?: () => void): Promise<void> {
  ...
  // Watch both Claude (.claude) and native (.cougar) directories
  const userClaudeDir = join(homedir(), '.claude', 'agents')
  const userCougarDir = join(homedir(), '.cougar', 'agents')
  const projectClaudeDir = join(getCwd(), '.claude', 'agents')
  const projectCougarDir = join(getCwd(), '.cougar', 'agents')
  ...
  watchDirectory(userCougarDir, 'user/.cougar')
  watchDirectory(projectClaudeDir, 'project/.claude')
  watchDirectory(projectCougarDir, 'project/.cougar')
}
