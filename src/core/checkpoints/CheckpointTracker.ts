...
// CLI-specific types
interface FolderLockWithRetryResult {
	acquired: boolean
	skipped: boolean
}
...
class CheckpointTracker {
	...
	/**
	 * CLI: Checkpoint event logging (replaces VS Code event system)
	 */
	private logCheckpointEvent(
		...
		const status = isActive ? 'Started' : 'Completed'
		const hashInfo = commitHash ? ` [${commitHash.substring(0, 8)}]` : ''
		console.debug(`[Checkpoint] ${operation} ${status}${hashInfo}`)
	}
	...
	public static async create(
		...
		try {
			...
			newTracker.logCheckpointEvent("CHECKPOINT_INIT", true)
			...
			newTracker.logCheckpointEvent("CHECKPOINT_INIT", false)

			const durationMs = Math.round(performance.now() - startTime)
			console.debug(`[Checkpoint] Shadow git initialized in ${durationMs}ms`)
	...
	public async commit(): Promise<string | undefined> {
		...
		try {
			this.logCheckpointEvent("CHECKPOINT_COMMIT", true)
			...
			const durationMs = Math.round(performance.now() - startTime)
			this.logCheckpointEvent("CHECKPOINT_COMMIT", false, commitHash)
			console.debug(`[Checkpoint] Commit created in ${durationMs}ms`)
	...
	public async resetHead(commitHash: string): Promise<void> {
		...
		try {
			...
			const startTime = performance.now()
			this.logCheckpointEvent("CHECKPOINT_RESTORE", true, commitHash)
			const lockResult: FolderLockWithRetryResult = await tryAcquireCheckpointLockWithRetry(this.cwdHash, this.taskId)
			...
			const durationMs = Math.round(performance.now() - startTime)
			this.logCheckpointEvent("CHECKPOINT_RESTORE", false, commitHash)
			console.debug(`[Checkpoint] Restored in ${durationMs}ms`)
		} catch (error) {
	...
	> {
		...
		const durationMs = Math.round(performance.now() - startTime)
		console.debug(`[Checkpoint] Diff generated in ${durationMs}ms`)
