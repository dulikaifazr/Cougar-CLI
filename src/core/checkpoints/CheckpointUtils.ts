...
/**
 * Gets the path to the shadow Git repository in CLI storage.
 *
 * CLI Checkpoints path structure:
 * ~/.cline-cli/
 *   checkpoints/
 *     {cwdHash}/
 *       .git/
...
/**
 * Gets the current working directory for CLI.
...
export async function getWorkingDirectory(): Promise<string> {
	// CLI 版本: 使用 process.cwd()
	const cwd = process.cwd()
	if (!cwd) {
		throw new Error("No workspace detected. Please run command in a valid directory.")
	}
