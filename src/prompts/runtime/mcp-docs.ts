// 核心作用：当 AI 需要创建或编辑 MCP 服务器时，提供完整的教程和示例代码
...
/**
 * 加载 MCP 文档
 * @param config MCP 配置信息
 */
export async function loadMcpDocumentation(config: McpConfig) {
...
Unless the user specifies otherwise, new MCP servers should be created in: ${config.mcpServersPath}
...
5. Install the MCP Server by adding the MCP server configuration to the settings file located at '${config.mcpSettingsFilePath}'. The settings file may have other MCP servers already configured, so you would read it first and then add your new server to the existing \`mcpServers\` object.
